...
def search_products(query: str, limit: int = 10) -> List[Dict]:
    ...
    Args:
        query: The search query (e.g., product name, category, or "all" for all products).
    ...
    params = {
        ...
        "status": "publish"  # 只返回已发布的商品
    }
    
    # 如果 query 不是要求"所有商品"，才添加 search 参数
    # 检测常见的"所有"、"全部"等关键词
    query_lower = query.lower()
    all_keywords = ['all', 'everything', '所有', '全部', '所有的', '全部的', '全部商品', '所有商品']
    
    if not any(keyword in query_lower for keyword in all_keywords):
        # 只有在不是查询所有商品时，才使用 search 参数
        params["search"] = query
        logger.info(f"🔍 Searching with keyword: {query}")
    else:
        logger.info(f"📋 Retrieving all products (no search filter)")
        ...
        try:
            ...
            for product in products:
                simplified_products.append({
                    ...
                    "description": product.get("short_description") or (product.get("description", "")[:100] + "..." if product.get("description") else ""),
