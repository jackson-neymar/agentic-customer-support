...
from customer_support_chat.app.core.audit_logger import audit_logger
...
from customer_support_chat.app.services.assistants.weather_search_assistant import (
  weather_search_assistant,
)
...
def pop_dialog_state(state: State) -> dict:
    """Pop the top of the dialog_state stack and hand control back to the primary assistant.

    Triggered after a sub-assistant invokes CompleteOrEscalate. Returning
    {"dialog_state": "pop"} causes update_dialog_stack() in state.py to drop the
    current frame so subsequent routing reflects the actual conversational context.
    """
    dialog_stack = state.get("dialog_state", []) or []
    logger.info(
        f"🪝 pop_dialog_state: popping top of dialog stack (before pop: {dialog_stack})"
    )
    messages = []
    last_message = state["messages"][-1] if state.get("messages") else None
    # If the last message is an AI message that still has open tool_calls,
    # acknowledge them so the LLM history stays consistent. In normal flow the
    # tool node has already produced a ToolMessage, so this branch is a safety net.
...
def route_update_flight_tools(state: State) -> Literal["update_flight", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "update_flight"

def route_car_rental_tools(state: State) -> Literal["book_car_rental", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "book_car_rental"

def route_hotel_tools(state: State) -> Literal["book_hotel", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "book_hotel"

def route_excursion_tools(state: State) -> Literal["book_excursion", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "book_excursion"
...
def guardrail_check(state: State, config: RunnableConfig):
    ...
    if not user_messages:
        ...
        guardrail_check_duration.observe(time.time() - start_time)
        return {"messages": []}
    ...
    try:
        ...
        logger.error(f"❌ Jailbreak guardrail invocation failed: {e}. Allowing message.")
        jailbreak_result = None

    if jailbreak_result is not None and not jailbreak_result.is_safe:
        logger.warning(f"🚨 Jailbreak attempt detected: {jailbreak_result.reasoning}")
        guardrail_blocks_total.labels(type="jailbreak").inc()
        audit_logger.log_jailbreak_attempt(
            session_id, user_input, jailbreak_result.reasoning
        )
        guardrail_check_duration.observe(time.time() - start_time)
        return {
            "messages": [
                AIMessage(
                    content=(
                        ...
                        f"Reason: {jailbreak_result.reasoning}"
                    )
                )
            ]
        }
    ...
    try:
        ...
        logger.error(f"❌ Relevance guardrail invocation failed: {e}. Allowing message.")
        relevance_result = None

    if relevance_result is not None and not relevance_result.is_relevant:
        logger.warning(f"⚠️ Irrelevant input detected: {relevance_result.reasoning}")
        guardrail_blocks_total.labels(type="irrelevant").inc()
        audit_logger.log_irrelevant_message(
            session_id, user_input, relevance_result.reasoning
        )
        guardrail_check_duration.observe(time.time() - start_time)
        return {
            "messages": [
                AIMessage(
                    content=(
                        "I can help you with travel-related queries (flights, hotels, "
                        "car rentals, excursions), e-commerce products and orders, "
                        "form submissions, and blog/weather searches. Your query seems "
                        f"unrelated. Reason: {relevance_result.reasoning}"
                    )
                )
            ]
        }

    # If both checks pass, the input is safe and relevant.
    duration = time.time() - start_time
    guardrail_check_duration.observe(duration)
    audit_logger.log_guardrail_pass(session_id, duration * 1000)
...
def route_woocommerce_tools(state: State) -> Literal["woocommerce", "leave_skill"]:
    logger.info(f"🤖 WooCommerce tools routing check")
    if should_route_to_primary(state):
        logger.info(f"➡️ Routing from WooCommerce tools back to primary assistant via leave_skill")
        return "leave_skill"
    else:
...
def route_form_submission_tools(state: State) -> Literal["form_submission", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "form_submission"
...
def route_blog_search_tools(state: State) -> Literal["blog_search", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "blog_search"
...
# Weather Search Assistant
def route_weather_search(state: State) -> Literal[
  "weather_search_safe_tools",
  "primary_assistant",
  "__end__",
]:
  route = tools_condition(state)
  if route == END:
      return END
  return "weather_search_safe_tools"


def route_weather_search_tools(state: State) -> Literal["weather_search", "leave_skill"]:
    return "leave_skill" if should_route_to_primary(state) else "weather_search"

builder.add_node("enter_weather_search",
                 create_entry_node("Weather Search Assistant","weather_search")
                 )
builder.add_node("weather_search",weather_search_assistant)
builder.add_edge("enter_weather_search","weather_search")
# 创建工具节点
builder.add_node(
  "weather_search_safe_tools",
  create_tool_node_with_fallback([search_weather_posts, CompleteOrEscalate]), # Include blog search tool
)

builder.add_conditional_edges("weather_search",route_weather_search)
builder.add_conditional_edges("weather_search_safe_tools",route_weather_search_tools)
...
# leave_skill: pops the current sub-assistant frame from dialog_state and
# routes back to the primary assistant. This guarantees that every sub-assistant
# entry (push via create_entry_node) has a matching pop, keeping the stack
# consistent across CompleteOrEscalate-driven hand-offs.
...
def route_primary_assistant(state: State) -> Literal[
  ...
  "enter_blog_search",
  "enter_weather_search", # New route
  "__end__",
]:
  # Reference the current dialog_state stack for context-aware logging /
  # auxiliary decision-making. The stack tells us which sub-assistant context
  # the user was in before returning to the primary assistant.
  dialog_stack = state.get("dialog_state", []) or []
  if dialog_stack:
      logger.debug(
          f"📚 route_primary_assistant: active dialog_state stack = {dialog_stack}, "
          f"top-of-stack = {dialog_stack[-1]}"
      )
  else:
      logger.debug("📚 route_primary_assistant: dialog_state stack is empty")
  ...
  if tool_calls:
      ...
      elif tool_name == ToWeatherSearch.__name__:
          return "enter_weather_search"
      else:
...
builder.add_conditional_edges(
  ...
  route_primary_assistant,
  {
      ...
      "enter_blog_search": "enter_blog_search", # New edge
      "enter_weather_search": "enter_weather_search", # New edge
...
# Route from guardrail check: if blocked (AIMessage appended), end; otherwise continue.
def route_guardrail(state: State) -> Literal["primary_assistant", "__end__"]:
    """If guardrail produced a blocking AIMessage, terminate; otherwise hand off to primary assistant."""
